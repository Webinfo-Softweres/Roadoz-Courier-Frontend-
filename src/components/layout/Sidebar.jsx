...
export function Sidebar({ isOpen, setIsOpen }) {
  ...
  // Helper to check permission against the Redux state
  ...
  const sections = {
    ...
    tools: [
      { name: "Serviceable Pincode", to: `${base}/serviceable-pincode`, perm: "orders:view" }, // Falling back to orders:view as tools:view is not in JSON
      { name: "Rate Calculator", to: `${base}/rate-calculator`, perm: "orders:view" },
      { name: "Channel Integration", to: `${base}/channel-integration`, perm: "orders:view" },
    ],
    finance: [
    ...
    trip: [
      { name: "Drivers", to: `${base}/trip/drivers`, perm: "drivers:view" },
      { name: "Vehicles", to: `${base}/trip/vehicles`, perm: "vehicle:view" },
      { name: "Trip Sheet", to: `${base}/trip/trip-sheet`, perm: "tripsheet:view" },
      { name: "Delivery Bookings", to: `${base}/trip/delivery-bookings`, perm: "tripsheet:view" },
      { name: "Pickup Bookings", to: `${base}/trip/pickup-bookings`, perm: "tripsheet:view" },
    ],
  };
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      toast.error("Logout issue", { id: loadingToast });
      navigate("/login");
    }
  };
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto py-4 space-y-1 custom-scrollbar">
       ...
       {/* NEW PARCEL CREATION */}
        ...
        {hasPerm("drivers:view") && (
          <NavLink
            to={`${base}/driver-tracking`}
        ...
        {/* Note: rto_addresses:view was not in JSON, using pickup_addresses:view as logical fallback */}
        {hasPerm("pickup_addresses:view") && (
